#!/usr/bin/env node
/**
 * Test: examples/full-workflow.yml
 *
 * Simulates the full setup — 5 URLs, mobile+desktop, all features.
 * Uses 1 run locally to keep it fast; CI uses 3.
 *
 * Usage: node scripts/__tests__/test-full-workflow.mjs
 */
import { requireBundle, runAction } from './_helper.mjs'

requireBundle()

// Exact URLs from examples/full-workflow.yml (replace your-site.com with real site)
const urls = [
  { url: 'https://yasinates.com/',          name: 'Home' },
  { url: 'https://yasinates.com/hakkimda',  name: 'About' },
  { url: 'https://yasinates.com/blog',      name: 'Blog' },
  { url: 'https://yasinates.com/projeler',  name: 'Projects' },
  { url: 'https://yasinates.com/iletisim',  name: 'Contact' },
]

console.log(`\n🧪 full-workflow.yml — ${urls.length} pages\n`)

// Exact parameters from examples/full-workflow.yml
