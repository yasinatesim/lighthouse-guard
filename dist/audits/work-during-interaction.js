...
// node_modules/lighthouse/core/audits/work-during-interaction.js
var TASK_THRESHOLD = 1;
var UIStrings = {
...
var WorkDuringInteraction = class _WorkDuringInteraction extends Audit {
  static {
    __name(this, "WorkDuringInteraction");
  }
  /**
  ...
  static recursivelyClipTasks(task, parent, startTs, endTs) {
    ...
    task.startTime = Math.max(startTs, Math.min(endTs, taskEventStart)) / 1e3;
    task.endTime = Math.max(startTs, Math.min(endTs, taskEventEnd)) / 1e3;
    task.duration = task.endTime - task.startTime;
    const childTime = task.children.map((child) => _WorkDuringInteraction.recursivelyClipTasks(child, task, startTs, endTs)).reduce((sum, child) => sum + child, 0);
  ...
  static clipTasksByTs(tasks, startTs, endTs) {
    for (const task of tasks) {
      ...
      _WorkDuringInteraction.recursivelyClipTasks(task, void 0, startTs, endTs);
    }
  }
  /**
  ...
  static getPhaseTimes(interactionEvent) {
    ...
    const navStart = startTs - interactionData.timeStamp * 1e3;
    const processingStartTs = navStart + interactionData.processingStart * 1e3;
    const processingEndTs = navStart + interactionData.processingEnd * 1e3;
    const endTs = startTs + interactionData.duration * 1e3;
    return {
  ...
  static getThreadBreakdownTable(interactionEvent, trace, processedTrace, networkRecords) {
    ...
    const phases = _WorkDuringInteraction.getPhaseTimes(interactionEvent);
    const items = [];
    for (const [phaseName, phaseTimes] of Object.entries(phases)) {
      _WorkDuringInteraction.clipTasksByTs(threadTasks, phaseTimes.startTs, phaseTimes.endTs);
      ...
      for (const [url, timingByGroupId] of executionTimings) {
        const totalExecutionTimeForURL = Object.values(timingByGroupId).reduce((total, timespanMs) => total + timespanMs);
      ...
      const filteredResults = results.filter((result) => result.total > TASK_THRESHOLD).sort((a, b) => b.total - a.total);
      items.push({
        phase: str_(UIStrings[
          /** @type {keyof UIStrings} */
          phaseName
        ]),
        total: (phaseTimes.endTs - phaseTimes.startTs) / 1e3,
        subItems: {
          type: "subitems",
  ...
  static async audit(artifacts, context) {
    ...
    const traceElementItem = _WorkDuringInteraction.getTraceElementTable(artifacts.TraceElements);
    ...
    const { table: breakdownTable, phases } = _WorkDuringInteraction.getThreadBreakdownTable(
      interactionEvent,
      trace,
      processedTrace,
      networkRecords
    ...
    const passed = duration < interaction_to_next_paint_default.defaultOptions.p10;
    return {
      score: passed ? 1 : 0,
...
var work_during_interaction_default = WorkDuringInteraction;
export {
  UIStrings,
  work_during_interaction_default as default
};
