...
// node_modules/lighthouse/core/audits/autocomplete.js
var UIStrings2 = {
  ...
  description: "`autocomplete` helps users submit forms quicker. To reduce user effort, consider enabling by setting the `autocomplete` attribute to a valid value. [Learn more about `autocomplete` in forms](https://developers.google.com/web/fundamentals/design-and-ux/input/forms#use_metadata_to_enable_auto-complete)",
...
var predictionTypesToTokens = {
  "NO_SERVER_DATA": str_(UIStrings2.manualReview),
  "UNKNOWN_TYPE": str_(UIStrings2.manualReview),
  "EMPTY_TYPE": str_(UIStrings2.manualReview),
  ...
  "HTML_TYPE_UNSPECIFIED": str_(UIStrings2.manualReview),
  "HTML_TYPE_NAME": "name",
  ...
  "HTML_TYPE_UPI_VPA": str_(UIStrings2.manualReview),
  "HTML_TYPE_ONE_TIME_CODE": "one-time-code",
  "HTML_TYPE_UNRECOGNIZED": str_(UIStrings2.manualReview),
  "HTML_TYPE_TRANSACTION_AMOUNT": "transaction-amount",
  "HTML_TYPE_TRANSACTION_CURRENCY": "transaction-currency"
};
var AutocompleteAudit = class extends Audit {
  static {
    __name(this, "AutocompleteAudit");
  }
  /**
  ...
  static audit(artifacts) {
    ...
    for (const input of artifacts.Inputs.inputs) {
      ...
      if (noPrediction.includes(input.autocomplete.prediction) && !input.autocomplete.attribute) continue;
      ...
      if (input.autocomplete.attribute) {
        warnings.push(str_(UIStrings2.warningInvalid, {
          token: input.autocomplete.attribute,
          snippet: input.node.snippet
        }));
      }
      if (validity.isValidOrder === false) {
        warnings.push(str_(UIStrings2.warningOrder, {
          tokens: input.autocomplete.attribute,
          snippet: input.node.snippet
        }));
        suggestion = UIStrings2.reviewOrder;
      }
      if (!(input.autocomplete.prediction in predictionTypesToTokens) && validity.isValidOrder) {
        lighthouse_logger_default.warn(`Autocomplete prediction (${input.autocomplete.prediction})
    ...
    const headings = [
      ...
      { key: "current", valueType: "text", label: str_(UIStrings2.columnCurrent) },
      { key: "suggestion", valueType: "text", label: str_(UIStrings2.columnSuggestions) }
    ];
    const details = Audit.makeTableDetails(headings, failingFormsData);
    let displayValue;
    if (failingFormsData.length > 0) {
      displayValue = str_(
        UIStrings.displayValueElementsFound,
...
var autocomplete_default = AutocompleteAudit;
export {
...
lighthouse/core/audits/autocomplete.js:
  (**
   * @license
